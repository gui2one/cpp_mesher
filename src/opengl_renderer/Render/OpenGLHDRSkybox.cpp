...
namespace GLR {
	...
	void HDRSkybox::Init() {
		...
		std::filesystem::path shaders_dir = "Resources/Shaders/irradiance";

		equirectangularToCubemapShader = std::make_shared<Shader>(
			shaders_dir / "2.2.2.cubemap.vs", 
			shaders_dir / "2.2.2.equirectangular_to_cubemap.fs");
		irradianceShader = std::make_shared<Shader>(
			shaders_dir / "2.2.2.cubemap.vs", 
			shaders_dir / "2.2.2.irradiance_convolution.fs");
		prefilterShader = std::make_shared<Shader>(
			shaders_dir / "2.2.2.cubemap.vs",
			shaders_dir / "2.2.2.prefilter.fs");
		brdfShader = std::make_shared<Shader>(
			shaders_dir / "2.2.2.brdf.vs",
			shaders_dir / "2.2.2.brdf.fs");
		backgroundShader = std::make_shared<Shader>(
			shaders_dir / "2.2.2.background.vs", 
			shaders_dir / "2.2.2.background.fs");
		...
		for (unsigned int i = 0; i < 6; ++i)
		{
			glUniformMatrix4fv(glGetUniformLocation(equirectangularToCubemapShader->GetID(), "view"), 1, GL_FALSE, glm::value_ptr(captureViews[i]));
