...
{
    uint32_t a = uint32_t((uint8_t)(clr[0]) << 0 |
                            (uint8_t)(clr[1]) << 8 |
                            (uint8_t)(clr[2]) << 16 |
                            (uint8_t)(0));        
    return a;
...
void OpenGLRenderer::Init(){
    ...
    if( mat != nullptr)
    {
        ...
        normal_map->Load("Resources/Textures/flat_normal.png");
...
void OpenGLRenderer::InitTextures()
{
    ...
    white_texture->Load("Resources/Textures/white.png");
    
    uv_grid_texture = std::make_shared<Texture>();
    uv_grid_texture->Load("Resources/Textures/uv_grid.jpg");        
}
...
void OpenGLRenderer::CollectLightsUniforms(const Scene &scene)
{
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(DirectionalLightUniform) * dir_light_uniforms.size(), dir_light_uniforms.data(), GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(SpotLightUniform) * spot_light_uniforms.size(), spot_light_uniforms.data(), GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(PointLightUniform) * point_light_uniforms.size(), point_light_uniforms.data(), GL_DYNAMIC_DRAW);
...
{
    uint32_t offset = 10; /* offset to give enough space for actual material texture channels ( i.e roughness, normal_map ... ) */
...
void OpenGLRenderer::RenderShadowPass(std::shared_ptr<Layer> layer, const Scene &scene, std::shared_ptr<Camera> camera)
{
    ...
    for(auto& light : scene.GetLights())
    {
        ...
        }else if(point_ptr)
        {
            ...
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3( 1.0, 0.0, 0.0), glm::vec3(0.0,-1.0, 0.0)));
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0,-1.0, 0.0)));
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3( 0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0)));
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3( 0.0,-1.0, 0.0), glm::vec3(0.0, 0.0,-1.0)));
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3( 0.0, 0.0, 1.0), glm::vec3(0.0,-1.0, 0.0)));
            shadowTransforms.push_back(shadowProj * 
                            glm::lookAt(point_ptr->GetPosition(), point_ptr->GetPosition() + glm::vec3( 0.0, 0.0,-1.0), glm::vec3(0.0,-1.0, 0.0)));
            ...
            for(const auto& object : scene.GetObjects())
            {
                ...
                /* 
                    for sources see : $project_dir/Resources/Shaders/cube_depth_shader.geo 
                */
...
void OpenGLRenderer::RenderObjects(std::shared_ptr<Layer> layer, const Scene &scene, std::shared_ptr<Camera> camera)
{
    ...
    for(const auto& object : scene.GetFont2dObjects())
    {

        glDisable(GL_CULL_FACE);

        msdf_font_shader->UseProgram();
        ...
        object->GetAtlas()->GetTexture()->Bind(0);
        msdf_font_shader->SetInt("msdf", 0);        



        // msdf_font_shader->SetInt("u_numDirectionalLights", m_NumDirectionalLights);
        // msdf_font_shader->SetInt("u_numSpotLights", m_NumSpotLights);
        // msdf_font_shader->SetInt("u_numPointLights", m_NumPointLights);



        auto tr = object->ApplyParentTransforms();
        msdf_font_shader->SetVec3("u_objectColor", object->m_Color);
        msdf_font_shader->SetMat4("u_model", tr);
        msdf_font_shader->SetFloat("pxRange", 4.0);
        msdf_font_shader->SetMat4("u_projection", camera->m_Projection);
        msdf_font_shader->SetMat4("u_view", view_matrix);

        msdf_font_shader->SetVec3("u_viewPos", camera->GetPosition());

        auto clr = entity_id_to_color(object->m_ID);
        msdf_font_shader->SetVec3("u_ColorID", clr);        
   ...
   for(const auto& object : scene.GetObjects())
    {
        ...
        outline_shader->SetVec3("u_color", glm::vec3(0.0f, 0.5f, 1.0f));
        object->Render();
    }
