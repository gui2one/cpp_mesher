...
namespace GLR 
{
	...
	public:
		...
		std::vector<Mesh> LoadBinary(fs::path path);
		std::vector<std::shared_ptr<MeshObject>> LoadBinaryMeshObjects(fs::path path);

		static Mesh ConvertToOrbitalsMesh(tinygltf::Model& model, tinygltf::Mesh gltf_mesh);

	private:

		static GLTFLoader* s_Instance; 
